...
class ChunkedWriter:
    """HTTP/1.1 Transfer-Encoding: chunked 封装。
    CDN(如 Cloudflare)对没有 chunked 框架的流式响应会全量缓冲,
    导致 xhttp 下行数据直到连接关闭才吐出。必须用 chunked 分帧。"""
...
async def relay_down(reader, writer, session, resp_header):
    ...
    try:
        if resp_header:
            await writer.write_chunk(resp_header)
        while not session.cleaned:
            chunk = await reader.read(SETTINGS["CHUNK_SIZE"])
            if not chunk:
...
async def handle_get(reader, writer, sid, leftover=b""):
    ...
    # NOTE: 200 响应头必须在 init(POST seq=0)完成之后再发送。
    # 若立即发送,CDN(如 Cloudflare)会把该到源连接标记为"响应传输中",
    # 把同一会话的 POST 排队在其后 -> init 永远等不到 -> 死锁。
    # 另: 下行必须用 Transfer-Encoding: chunked 分帧,否则 CDN 会全量缓冲。
    writer.write(
        b"HTTP/1.1 200 OK\r\n"
